...
describe 'Jbuilder#Pagination' do
  ...
  context 'pagination calls pagination methods' do
    it('works') do
      call_count = 0
      test_pages = [[:page, ->(_params) { call_count += 1 }], [:per, ->(_params) { call_count += 1 }]]
      paginate(DummyPageable.new, test_pages, page: { size: 2 })
  ...
  context 'pagination raises error on unpageable resource' do
    it('actually works') do
      test_pages = [[:page, ->(params) { params }]]
      expect { paginate({ not: :pageable }, test_pages, page: { size: 2 }) }.to(
